(() => {
  ...
  function form(item) {
    ...
    dialog.querySelector('#productFormPrice')?.closest('label')?.insertAdjacentHTML('afterend', `<label>成本生效日期<input id="productCostEffectiveDate" type="date" value="${new Date().toISOString().slice(0, 10)}" required /><small>修改单价时会新增成本版本；历史订单按下单日期取对应版本。</small></label>`);
